...
import { FileOutput, Eye, Printer, CheckCircle, Clock, AlertTriangle, Edit } from 'lucide-react';
...
const pendingDischarges = [
  ...
  { id: 'IPD-2003', patient: 'Mohan Lal', uhid: 'P-1005', ward: 'Private-1', bed: 'B-01', admitted: '2024-03-08', days: 7, diagnosis: 'Cholecystectomy', doctor: 'Dr. Singh', billAmt: '₹1,80,000', deposit: '₹1,80,000', balance: '₹0', pharmacy: 'Cleared', lab: 'Cleared', billing: 'Cleared', summary: 'Summary Ready', status: 'Summary Ready' },
  { id: 'IPD-2004', patient: 'Priya Singh', uhid: 'P-1004', ward: 'Ward-B', bed: 'B-05', admitted: '2024-03-13', days: 2, diagnosis: 'Normal Delivery', doctor: 'Dr. Verma', billAmt: '₹45,000', deposit: '₹45,000', balance: '₹0', pharmacy: 'Cleared', lab: 'Cleared', billing: 'Cleared', summary: 'Summary Ready', status: 'Summary Ready' },
  { id: 'IPD-2005', patient: 'Amit Sharma', uhid: 'P-1003', ward: 'ICU-2', bed: 'B-01', admitted: '2024-03-14', days: 1, diagnosis: 'Post-Op Monitoring', doctor: 'Dr. Singh', billAmt: '₹35,000', deposit: '₹25,000', balance: '₹10,000', pharmacy: 'Pending', lab: 'Pending', billing: 'Pending Clearance', summary: 'Pending', status: 'Pending' },
];
...
const dischargedToday = [
  ...
  { id: 'IPD-1996', patient: 'Sunita Kumari', uhid: 'P-0999', ward: 'Ward-A', time: '09:15', diagnosis: 'Pneumonia', doctor: 'Dr. Sharma', totalBill: '₹42,000', paid: '₹42,000', type: 'Normal', followUp: '2024-03-20' },
  { id: 'IPD-1997', patient: 'Deepak Verma', uhid: 'P-1000', ward: 'ICU-1', time: '10:00', diagnosis: 'CVA', doctor: 'Dr. Gupta', totalBill: '₹3,50,000', paid: '₹3,20,000', type: 'Normal', followUp: '2024-03-18' },
];
...
const Discharge = () => {
  ...
  return (
    <div>
      <div className="hms-section-header flex items-center gap-2"><FileOutput size={14} /> Discharge & Summary Management</div>
      ...
      {tab === 'Dashboard' && (
        <div>
          <div className="grid grid-cols-6 gap-2 mb-3">
            {[{ l: 'Pending Discharge', v: '5', s: '2 Ready' },{ l: 'Discharged Today', v: '3', s: 'Normal: 3' },{ l: 'Billing Pending', v: '3', s: '₹90,000 Due' },{ l: 'Summary Pending', v: '3', s: '2 > 24hrs' },{ l: 'LAMA This Month', v: '2', s: '0.8%' },{ l: 'Avg LOS', v: '4.2 Days', s: 'Target: 4.0' }].map((k, i) => (
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-card border border-border">
              ...
              <table className="hms-table"><thead><tr><th>IPD No</th><th>Patient</th><th>Ward</th><th>Days</th><th>Pharmacy</th><th>Lab</th><th>Billing</th><th>Summary</th><th>Status</th></tr></thead>
                <tbody>{pendingDischarges.map(p => <tr key={p.id}><td className="font-mono text-[10px]">{p.id}</td><td>{p.patient}</td><td>{p.ward}</td><td>{p.days}</td><td><StatusBadge status={p.pharmacy} /></td><td><StatusBadge status={p.lab} /></td><td><StatusBadge status={p.billing} /></td><td><StatusBadge status={p.summary} /></td><td><StatusBadge status={p.status} /></td></tr>)}</tbody>
              </table>
            </div>
            <div className="bg-card border border-border">
              ...
              <table className="hms-table"><thead><tr><th>IPD</th><th>Patient</th><th>Time</th><th>Diagnosis</th><th>Bill</th><th>Type</th><th>Follow-Up</th></tr></thead>
                <tbody>{dischargedToday.map(d => <tr key={d.id}><td className="font-mono text-[10px]">{d.id}</td><td>{d.patient}</td><td>{d.time}</td><td>{d.diagnosis}</td><td>{d.totalBill}</td><td>{d.type}</td><td>{d.followUp}</td></tr>)}</tbody>
              </table>
            </div>
          </div>
        </div>
      ...
      {tab === 'Discharged Today' && (
        <div>
          <table className="hms-table"><thead><tr><th>IPD No</th><th>Patient</th><th>UHID</th><th>Ward</th><th>Time</th><th>Diagnosis</th><th>Doctor</th><th>Total Bill</th><th>Paid</th><th>Type</th><th>Follow-Up</th><th>Action</th></tr></thead>
            <tbody>{dischargedToday.map(d => <tr key={d.id}><td className="font-mono text-[10px]">{d.id}</td><td>{d.patient}</td><td>{d.uhid}</td><td>{d.ward}</td><td>{d.time}</td><td>{d.diagnosis}</td><td>{d.doctor}</td><td>{d.totalBill}</td><td>{d.paid}</td><td>{d.type}</td><td>{d.followUp}</td><td><Eye size={12} className="text-primary cursor-pointer" /> <Printer size={12} className="text-muted-foreground cursor-pointer" /></td></tr>)}</tbody>
          </table>
        </div>
      )}
