          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-card border border-border">
              ...
              <table className="hms-table"><thead><tr><th>Ward</th><th>Total</th><th>Occupied</th><th>Vacant</th><th>Reserved</th><th>Occupancy%</th></tr></thead>
                <tbody>{wardBeds.map(w => <tr key={w.ward}><td>{w.ward}</td><td>{w.total}</td><td>{w.occupied}</td><td className="font-bold text-green-700">{w.vacant}</td><td>{w.reserved}</td><td>{Math.round(w.occupied / w.total * 100)}%</td></tr>)}</tbody>
              </table>
            </div>
          </div>
        </div>
      ...
      {tab === 'Bed Map' && (
        <div>
          <div className="flex gap-2 mb-2">
            <select className="hms-select"><option>All Wards</option>{wardBeds.map(w => <option key={w.ward}>{w.ward}</option>)}</select>
          ...
          <table className="hms-table"><thead><tr><th>Ward</th><th>Total</th><th>Occupied</th><th>Vacant</th><th>Reserved</th><th>Cleaning</th><th>Occupancy</th></tr></thead>
            <tbody>{wardBeds.map(w => <tr key={w.ward}><td className="font-bold">{w.ward}</td><td>{w.total}</td><td>{w.occupied}</td><td className="font-bold text-green-700">{w.vacant}</td><td>{w.reserved}</td><td>{w.cleaning}</td><td>{Math.round(w.occupied / w.total * 100)}%</td></tr>)}</tbody>
          </table>
        </div>
      )}
